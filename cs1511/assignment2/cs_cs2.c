...
#define CURRENTLY_SELECTED 1
#define NOT_CURRENTLY_SELECTED 0
////////////////////////////////////////////////////////////////////////////////
/////////////////////////// USER DEFINED TYPES  ////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
int check_space(struct carriage *destination_carriage, int num_move);
...
int main(void) {
    ...
    while (valid == 1) {
        if (command == '?') {
            ...
            int row = carriage_inputs + 1;
            int col = ID_SIZE;
            

            if (check_positive_int(carriage_inputs) == TRUE) {
                char (*carriage_ids)[row][col] = malloc(sizeof(*carriage_ids));
                printf("Enter ids:\n");
                for (int i = 0; i < row - 1; i++) {
                    ...
                    // carriage_ids[i][col] = '\0';
                }
                // multiple same inputs cause multiple splits when not meant to
                ...
                if (current_carriage != NULL) {
                    ...
                    while (current_carriage != NULL) {
                        int matched = FALSE;
                        int count = 0;
                        while (matched == FALSE && count < row - 1) {
                            char id[ID_SIZE];
                            for (int j = 0; j < col; j++) {
                                id[j] = (*carriage_ids)[count][j];
                            }
                            if (current_carriage != NULL && strcmp(current_carriage->carriage_id, id) == 0) {
                                ...
                                current_carriage = previous_carriage->next;
...
int check_carriage(char id[ID_SIZE], 
    ...
    if (type == INVALID_TYPE) {
        ...
        while (check_carriage != NULL && valid == TRUE) {
            if (strcmp(id, check_carriage->carriage_id) == 0) {
...
void insert_carriage(char id[ID_SIZE],
    ...
    if (position == 0 || current == NULL) {
        ...
        int index = 0;
        //want to stop 1 carriage behind intended one
...
struct carriage *find_passenger_carriage(struct train *train, char id[ID_SIZE], char command) {
    struct carriage *carriage = train->carriages;

    if (carriage != NULL) {
        while (strcmp(carriage->carriage_id, id) != 0 && carriage->next != NULL) {
            carriage = carriage->next;
        }  
        if (carriage->next == NULL && strcmp(carriage->carriage_id, id) != 0) {
            carriage = carriage->next;
        }
    }
    if (carriage == NULL && command != SPLIT_TRAIN) {
...
int check_before_moving(struct train *train, 
    ...
    if (check_positive_int(passengers) == TRUE) {
        ...
        if (carriage == NULL) {}
        else if (passengers > carriage->occupancy && command == REMOVE_PASSENGERS) {
    ...
    return valid;
    // ASK IF YOU CAN OVERLOAD A FUNCTION TO REDUCE REPEITION 
    // IE i want to make it so that i pass in command and then only else statements
    // change to perform whichever procedure required 
    //(since most input/error_check the same)
}
...
struct count_details count_passengers(struct carriage_range carriage) {
    ...
    while (current->next != NULL && include == 1) {
    
        total.occupancy += current->occupancy;
        total.capacity += current->capacity;
        
        if (carriage.move_to != NULL) {
            if (strcmp(current->carriage_id, (carriage.move_to)->carriage_id) == 0) {
                include = FALSE;
            }
        }
    ...
    if (carriage.move_to == NULL) {
        ...
        include = FALSE;
            ...
            (strcmp(current->carriage_id, (carriage.move_to)->carriage_id) == 0 ||
            strcmp(current->carriage_id, (carriage.move_from)->carriage_id) == 0)) {
        total.occupancy += current->occupancy;
        total.capacity += current->capacity;
...
void move_passengers_command(struct train *train, char command) {
    ...
    int source_valid = check_before_moving(train, REMOVE_PASSENGERS, num_move, source_id);
    ...
    if (source_valid == TRUE) {
        ...
        if (destination_valid == TRUE) {
            ...
            destination_carriage = find_passenger_carriage(train, destination_id, command);
        }
    }

    if (destination_carriage != NULL) {
        if (check_space(destination_carriage, num_move) == TRUE) {
...
void merge_trains(struct train *selected) {
    ...
    while (other_carriage != NULL) {
        int matching = FALSE;
        struct carriage *current_carriage = selected->carriages;
        while (current_carriage != NULL && matching == FALSE) {
            if (strcmp(current_carriage->carriage_id, 
                        other_carriage->carriage_id) == 0) {
                matching = TRUE;
...
void search_train_to_split(struct train *selected,
        ...
        int row);
int check_id_to_split(struct carriage *current_carriage,
...
void search_train_to_split(struct train *selected, 
        ...
        int row) {
    struct carriage *current_carriage = selected->carriages;
    struct carriage *presvious_carriage = selected->carriages;
    struct train *current_train = selected;
    int deleted = 0;
    ...
    while (current_carriage != NULL) {
        ...
        while (current_row < (row - deleted) && matched == FALSE) {
            ...
            matched = check_id_to_split(current_carriage, previous_carriage, 
                            current_train, id, carriage_ids, row, current_row, deleted);
            deleted += matched;
            current_row++;
        }
...
struct carriage *find_passenger_carriage(struct train *train, 
    ...
    if (carriage != NULL) {
        while (strcmp(carriage->carriage_id, id) != 0 && 
                carriage->next != NULL) {
            carriage = carriage->next;
        }  
...
void clear_memory(struct head_train *head_train) {
    struct train *current = head_train->head;
    // Loops till second-last train, cause if current != NULL, will
    // constantly create new train to delete due to function.
    while (current->next != NULL) {
        current = remove_train(head_train, current);
    }
...
void search_train_to_split(struct train *selected, 
    ...
    while (current_carriage != NULL) {
        ...
        while (current_row < (row - deleted) && matched == FALSE) {
            ...
            if (current_carriage != NULL && 
                    strcmp(current_carriage->carriage_id, id) == 0) {
