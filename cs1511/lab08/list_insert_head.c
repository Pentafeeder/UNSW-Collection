// Takes initial list and allows user to insert a new head before printing new list
...
struct node *insert_head(int value, struct node *head) {
    ...
    struct node *new_first_node = malloc(sizeof(struct node));
    new_first_node->next = head;
    new_first_node->data = value;
    head = new_first_node;
    return head;
