...
void lru(int n_physical_frames, int n_virtual_pages);
void access_page(int virtual_page, int access_time, int n_physical_frames, struct ipt_entry *ipt);

int main(int argc, char *argv[]) {
    if (argc != 3) {
        fprintf(stderr, "Usage: %s <n-physical-frames> <n-virtual-pages>\n", argv[0]);
        return 1;
    }
...
void lru(int n_physical_frames, int n_virtual_pages) {
    printf("Simulating %d frames of physical memory, %d pages of virtual memory\n",
          n_physical_frames, n_virtual_pages);
    struct ipt_entry *ipt = malloc(n_physical_frames * sizeof *ipt);
    assert(ipt);

    for (int i = 0; i < n_physical_frames; i++) {
        ipt[i].virtual_page = PAGE_FREE;
    ...
    for (int access_time = 0; scanf("%d", &virtual_page) == 1; access_time++) {
        ...
        access_page(virtual_page, access_time, n_physical_frames, ipt);
    }
}
