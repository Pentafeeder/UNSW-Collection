...
big_bcd_t *bcd_add(big_bcd_t *x, big_bcd_t *y) {
    ...
    // PUT YOUR CODE HERE
    // Always make x have smaller digit
    if (x->n_bcd > y->n_bcd) {
        int temp_n_bcd = x->n_bcd;
        x->n_bcd = y->n_bcd;
        y->n_bcd = temp_n_bcd;
        
        unsigned char *temp_bcd = x->bcd;
        x->bcd = y->bcd;
        y->bcd = temp_bcd;
    }

    unsigned char *digits = malloc(sizeof(unsigned char) * (y->n_bcd));
    ...
    for (int i = 0; i < y->n_bcd; i++) {
        ...
        if (i == y->n_bcd - 1 && carry > 0) {
            sum = realloc(sum, sizeof(int) * (y->n_bcd + 1));
            digits = realloc(digits, sizeof(int) * (y->n_bcd + 1));
            sum->n_bcd = y->n_bcd + 1;
            sum->bcd = digits;
            digits[y->n_bcd] = carry;
        }
    }
